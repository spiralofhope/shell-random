...
I'm not sure offhand what this was.  I assume this was an earlier version of mvcp.bat
...
cp mvcp.bat *.bak fails! it does not overwrite!!
...
that certain dirs can be skipped.
...
problem: cannot move .. . or . .. because %1 and %2 are both directories.  Just
mask those commands out.

in the far future:
if exist %2, compare size, if same, do a byte compare (does 4dos have an
	internal function, or will I have to use an outside program?)
if %1 = %2, delete %2
...
:		IFF EXIST %2 THEN
		...
		%$MVCP %&
:			? "Overwrite?" IF %@ATTRIB["%2",r,p]=1 (ATTRIB -R "%2"^%$MVCP %&)
:		ELSE
:			%$MVCP %&
:		ENDIFF
	ELSE
