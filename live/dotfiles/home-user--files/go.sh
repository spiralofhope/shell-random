...
if [ -z $1 ]; then
  dot='.'
  \ln  --force  --no-target-directory  --symbolic  "$PWD"/               ~/00--home-user--dotfiles
  \ln  --force  --no-target-directory  --symbolic  "$PWD"/descript.ion   ~/descript.ion
else
  \ln  --force  --no-target-directory  --symbolic  "$PWD"/               ~/00--home-user--files
  \ln  --force  --no-target-directory  --symbolic  "$PWD"/.descript.ion  ~/.descript.ion
fi
