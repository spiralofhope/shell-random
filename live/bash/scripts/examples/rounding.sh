...
:<<NOTE
This works for all the Wikipedia tests except for one 3.045 staying even (3.04) instead of being rounded up (3.05).
...
## Leave it the same if the next digit is 4 or less
if [ $digit_after -lt 5 ]; then echo $item ; return 0 ; fi
...
round() (
  ...
  until [ "sky" = "falling" ]; do
  if [ "$#" -lt 1 ] || [ "$#" -gt 2 ]; then echo "Needs a number and an optional round-to number" ; break ; fi
  isnumber $1 ; if [ -$? -ne 0 ] || [ "$1" = "" ]; then echo $1 is not a number. ; break ; fi
  if [ "$#" -eq 1 ]; then 
    roundto=0
  else
# 0 is not a number.  Boo.
#    isnumber $2 ; if [ $? -ne 0 ] || [ "$2" = "" ]; then echo $2 is not a number. ; break ; fi
    roundto=$2
  fi
